...
// MARK: โครงสร้าง GlobalData และ MarketDataModel นี้ถูกออกแบบมาเพื่อจัดการข้อมูลตลาดจาก API ของ CoinGecko โดยสามารถถอดรหัสข้อมูล JSON และคำนวณค่าต่าง ๆ ที่จำเป็นสำหรับการแสดงผลในแอปพลิเคชัน

// โครงสร้าง GlobalData ที่ประกอบด้วยตัวแปร data ซึ่งเป็นชนิด MarketDataModel? โดยใช้โปรโตคอล Codable เพื่อให้สามารถถอดรหัสข้อมูลจาก JSON ได้.
...
struct MarketDataModel: Codable {
    ...
    // แผนที่ (dictionary) ที่มีคีย์เป็น String และค่าของคีย์เป็น Double.
    let totalMarketCap, totalVolume, marketCapPercentage: [String: Double]
    
    // ค่าเปลี่ยนแปลงของมูลค่าตลาดใน 24 ชั่วโมง เป็น Double.
    let marketCapChangePercentage24HUsd: Double
    
    // เป็นการแมปชื่อคีย์ JSON กับชื่อ property ในโครงสร้าง.
    ...
    // คำนวณและคืนค่ามูลค่าตลาดทั้งหมดเป็นสตริงที่มีการจัดรูปแบบและเติมสัญลักษณ์ $ หากพบข้อมูลที่มีคีย์เป็น usd.
    ...
    // คำนวณและคืนค่าปริมาณการซื้อขายทั้งหมดเป็นสตริงที่มีการจัดรูปแบบและเติมสัญลักษณ์ $ หากพบข้อมูลที่มีคีย์เป็น usd.
    var volume: String {
    ...
    // คำนวณและคืนค่าส่วนแบ่งของมูลค่าตลาดที่เป็นของ Bitcoin (BTC) เป็นสตริงที่แสดงเปอร์เซ็นต์.
